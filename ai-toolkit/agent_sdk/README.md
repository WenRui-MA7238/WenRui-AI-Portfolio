...
├── src/
│   ├── __init__.py
│   ├── config.py            # Backward-compatible config class
│   ├── llm_backend.py       # Unified LLM backend wrapper
│   ├── agent.py             # SmartAgent main class
│   ├── prompts/
│   │   └── system_prompt.txt
│   ├── tools/
│   │   ├── __init__.py
│   │   ├── calculator.py    # Safe math evaluator
│   │   ├── search_kb.py     # RAG knowledge base integration
│   │   ├── weather.py       # Weather demo tool
│   │   └── web_search.py    # Web search demo tool
│   └── memory/
│       ├── __init__.py
│       └── session_memory.py  # Session-level memory store
├── scripts/
│   ├── run_agent.py         # CLI interaction
│   └── run_server.py        # FastAPI service entry
├── tests/
...
# Option 2: DashScope Qwen3
# LLM_PROVIDER=dashscope
# DASHSCOPE_API_KEY=sk-xxxx
# DASHSCOPE_MODEL=qwen3-30b-a3b

# Option 3: Ollama / vLLM local
# LLM_PROVIDER=ollama
